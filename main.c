...
/*
Secuencia "Ráfaga"
Esta secuencia enciende todos los LEDs de una vez, los mantiene encendidos por un corto periodo, y luego los apaga, repitiendo este ciclo.
...
void rafaga() {
    ...
    unsigned char output = 0xFF; // Todos los LEDs encendidos

    while (true) {
        ledShow(output); // Mostrar el valor en los LEDs
        disp_binary(output); // Mostrar el valor en binario en la pantalla

        if (delay(0) == 0) { // Esperar el retardo y verificar si se presionó una tecla
            turnOff(); // Apagar los LEDs
            return; // Salir de la secuencia
        }

        output = 0x00; // Apagar todos los LEDs
        ledShow(output); // Mostrar el valor en los LEDs
        disp_binary(output); // Mostrar el valor en binario en la pantalla
        ...
        output = 0xFF; // Encender todos los LEDs nuevamente
    }
}
...
rafaga:
    ...
    MOV R4, #0xFF  // Todos los LEDs encendidos
...
loop_rafaga:
    ...
    MOV R4, #0x00  // Apagar todos los LEDs
    MOV R0, R4
    BL disp_binary
    MOV R0, R4
    BL ledShow
    ...
    MOV R4, #0xFF  // Encender todos los LEDs nuevamente
    B loop_rafaga
...
Secuencia "Barrido Aleatorio"
Esta secuencia enciende y apaga los LEDs en un orden aleatorio, creando un efecto de barrido impredecible.
Implementación en C:
#include <stdlib.h> // Para la función rand()

void barridoAleatorio() {
    printf("\n--- Barrido Aleatorio ---\n");
    printf("Presione esc para finalizar la secuencia\n");
    printf("Presione W para aumentar la velocidad\n");
    printf("Presione S para disminuir la velocidad\n");
    ...
    while (true) {
        output = rand() % 256; // Generar un número aleatorio entre 0 y 255
        ledShow(output); // Mostrar el valor en los LEDs
        disp_binary(output); // Mostrar el valor en binario en la pantalla

        if (delay(1) == 0) { // Esperar el retardo y verificar si se presionó una tecla
            turnOff(); // Apagar los LEDs
            return; // Salir de la secuencia
        }
    }
...
.global barridoAleatorio
barridoAleatorio:
    PUSH {R4, LR}

loop_barridoAleatorio:
    BL random       // Llamar a la función que genera un número aleatorio
    AND R4, R0, #0xFF  // Asegurarse de que el número esté en el rango de 0 a 255
    MOV R0, R4
    BL disp_binary
    MOV R0, R4
    BL ledShow
    ...
    BL delay
    CMP R0, #0
    BEQ end_barridoAleatorio
    B loop_barridoAleatorio

end_barridoAleatorio:
    BL turnOff
    POP {R4, PC}
...
loop_rafaga:
    ...
    MOV R0, R4
    BL ledShow
    MOV R0, #0  // Index for array in C (adjust as necessary)
    BL delay
    CMP R0, #0
    BEQ end_rafaga
...
loop_barridoAleatorio:
    ...
    MOV R0, R4
    BL ledShow
    MOV R0, #1  // Index for array in C (adjust as necessary)
    BL delay
    CMP R0, #0
    BEQ end_barridoAleatorio
    B loop_barridoAleatorio
